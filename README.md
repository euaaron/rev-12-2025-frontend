...
The UI talks to a GraphQL endpoint at `/graphql` via Apollo Client, but during local development it’s intercepted by Mock Service Worker (MSW) — so we can run the app without a real backend.
This GitHub Pages is just for preview/showcase purpose, so it is also using MSW.
